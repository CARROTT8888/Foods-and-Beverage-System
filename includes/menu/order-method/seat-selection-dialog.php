...
<div class="fixed inset-0 bg-slate-950/50 flex justify-center items-center opacity-0 pointer-events-none transition-opacity duration-300 ease-out z-9999"
    ...
    <div class="bg-white rounded-xl shadow-2xl shadow-slate-950/5 border border-slate-200 w-108 scale-95">
        ...
        <div class="p-4 pt-2 text-slate-600 grid grid-cols-2 gap-2">
            <?php
            ...
            if ($tableResult->num_rows > 0):
                while ($data = $tableResult->fetch_assoc()):
                    ...
                    <?php if ($data['status'] === 'Available'): ?>
                        <button role="alert" type="button" id="table-<?php echo $data['tableId']; ?>"
                            onclick="selectTable(<?php echo $data['tableId']; ?>, 'Available')"
                            class="table-button relative flex w-full items-start rounded-none border border-b-0 border-l-4 border-r-0 border-t-0 border-green-500 bg-green-500/10 p-2 font-medium text-green-500">
                            ...
                            <div class="flex flex-col space-y-2">
                                <div class="m-1.5 w-full font-sans text-base leading-none">
                                    ...
                                    <span
                                        class="ml-20"><?php echo $data['availableSeat']; ?>/<?php echo $data['totalSeat']; ?></span>
                                </div>
                                <div data-shape="pill"
                    ...
                    <?php elseif ($data['status'] === 'Occupied'): ?>
                        <button role="alert" type="button" id="table-<?php echo $data['tableId']; ?>"
                            onclick="selectTable(<?php echo $data['tableId']; ?>, 'Occupied')" disabled
                            class="table-button relative flex w-full items-start rounded-none border border-b-0 border-l-4 border-r-0 border-t-0 border-red-500 bg-red-500/10 p-2 font-medium text-red-500">
                            <span class="grid shrink-0 place-items-center p-1">
                                <svg width="1.5em" height="1.5em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                                    color="currentColor" class="h-5 w-5">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <?php elseif ($data['status'] === 'Reserved'): ?>
                        <button role="alert" type="button" id="table-<?php echo $data['tableId']; ?>"
                            onclick="selectTable(<?php echo $data['tableId']; ?>, 'Reserved')" disabled
                            class="table-button relative flex w-full items-start rounded-none border border-b-0 border-l-4 border-r-0 border-t-0 border-orange-500 bg-orange-500/10 p-2 font-medium text-orange-500">
                            <span class="grid shrink-0 place-items-center p-1">
                                <svg width="1.5em" height="1.5em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                                    color="currentColor" class="h-5 w-5">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                    ...
                    <?php elseif ($data['status'] === 'Dirty'): ?>
                        <button role="alert" type="button" id="table-<?php echo $data['tableId']; ?>"
                            onclick="selectTable(<?php echo $data['tableId']; ?>, 'Dirty')"
                            class="table-button relative flex w-full items-start rounded-none border border-b-0 border-l-4 border-r-0 border-t-0 border-amber-500 bg-amber-500/10 p-2 font-medium text-amber-500">
                            <span class="grid shrink-0 place-items-center p-1">
                    ...
                    <?php elseif ($data['status'] === 'Blocked'): ?>
                        <button role="alert" type="button" id="table-<?php echo $data['tableId']; ?>"
                            onclick="selectTable(<?php echo $data['tableId']; ?>, 'Blocked')" disabled
                            class="table-button relative flex w-full items-start rounded-none border border-b-0 border-l-4 border-r-0 border-t-0 border-slate-500 bg-slate-500/10 p-2 font-medium text-slate-500">
                            <span class="grid shrink-0 place-items-center p-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
        ...
        <div class="p-4 flex justify-end gap-2">
            ...
            <button type="button" onclick="confirmBookSeat()"
        ...
        <script>
            function selectTable(tableId, status) {
                ...
                document.querySelectorAll('.table-button').forEach(button => button.classList.remove('ring-2', 'ring-primary'));
                document.getElementById('table-' + tableId).classList.add('ring-2', 'ring-primary');
            }

            function confirmBookSeat() {
                ...
                form.innerHTML = `
        <input type="hidden" name="methodName" value="Dine In">
