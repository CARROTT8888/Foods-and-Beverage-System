...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['deleteCategory'])) {
    ...
    if (empty($categoryId) || empty($branchId)) {
        echo "<script>alert('Invalid category or branch');</script>";
    } else {
        // check if category has food or drink
        $checkFoodQuery = "SELECT foodId FROM food WHERE categoryId = ? AND branchId = ?";
        $checkFoodStmt = $conn->prepare($checkFoodQuery);
        $checkFoodStmt->bind_param("ii", $categoryId, $branchId);
        $checkFoodStmt->execute();
        ...
        if ($checkFoodStmt->num_rows > 0) {
            ...
            // delete category
            $deleteQuery = "DELETE FROM food_category WHERE categoryId = ? AND branchId = ?";
            $stmt = $conn->prepare($deleteQuery);
            $stmt->bind_param("ii", $categoryId, $branchId);
    ...
    <div class="bg-white rounded-xl shadow-2xl shadow-slate-950/5 border border-slate-200 scale-95 w-115 p-5 ">
        <form method="POST">
            <div class="flex justify-between mb-4">
                <h1 class="text-lg text-slate-800 font-semibold">Deleting the Category</h1>
            ...
            <div class="mt-6">
                <div class="flex justify-end gap-2">
                    <button onclick="closeDeleteCategoryDialog()" type="button"
